...
export function SettingsRow({ title, subtitle, icon, color, value, onPress }: { title: string; subtitle?: string; icon: string; color?: string; value?: string; onPress?: () => void }) {
  ...
  return (
    <Pressable
      ...
      <View style={{ width: 38, height: 38, borderRadius: 13, backgroundColor: color ?? theme.accentContainer, alignItems: 'center', justifyContent: 'center' }}>
        <AppIcon name={icon} color={color ? '#FFFFFF' : theme.accent} size={18} />
      </View>
